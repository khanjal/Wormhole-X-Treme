...
- **The capture is the half-sphere a window can show, boxed:** `mirror-view-depth` plus two blocks ahead of the arrival point, to either side, up and down, and one block behind (`mirror-capture-radius` caps it). It keeps every block with a face open and the layer under it; what is buried deeper is marked buried, and the real world is left there. A capture smaller than the box it needs, or from before buried blocks were marked, is taken again on the next look.
- **For checking a mirror:** `/wormhole mirror debug <name> full` draws that mirror whole and without limits for you alone -- everything its capture holds, through the opening, past the edges and into the ground -- so you can see what the file holds and how it comes through. `mirror debug off` turns views off for you, so you see the world as it is; `mirror debug on` puts either back.

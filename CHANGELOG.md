...
- **`gate regen` keeps every setting `gate edit` made.** It kept the name, owner, iris code
  and network, and dropped the custom portal, iris and light materials, the woosh depth, redstone,
...
- **`gate edit <gate> group` lasts past a restart.** The choice was only held in memory, and the
  gate went back to its frame's group. `group default` gives it back to the frame.
